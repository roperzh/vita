class ProfileParser
  ...
  def initialize(profile)
    @profile = profile
    @fields = %w(
      languages skills certifications educations courses volunteer first_name
      last_name headline location summary positions picture-urls::(original)
      publications projects
    )
    @data = client.profile(fields: @fields)
  end
  ...
  def parse_date(ln_date)
    ...
    ln_date.month ? "#{ln_date.month}/#{ln_date.year}" : "#{ln_date.year}"
  end
end
